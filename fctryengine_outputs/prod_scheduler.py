        ...
        for row in self.data_dict['resource']:
            ...
            for sched in row['availability']:
                start_time = datetime.strptime(
                    sched['start_datetime'], '%Y-%m-%d %H:%M:%S%z')
                end_time = datetime.strptime(
                    sched['end_datetime'], '%Y-%m-%d %H:%M:%S%z')
                # needs to be not absent, and start and endtime is today and upcoming days
                if not sched['is_absent'] and start_time >= self.today and end_time >= self.today:
